...
{ 
    ...
    [Parameter(Position=6, Mandatory=$false)] [Int32]$MaxLength=8000,
...
## Row 181 can change the length of varchar, now set at 8000
...
Try { 
    ...
    # copy powerscript below


## Declaration of variables
$csvPath = "\\cyg7006\Data\P&R\Reporting\Data_Sources\CustomerCare\SourceV2\" 
$sqlserver = "Cyg7091"
$database = "CustomerCare" 
$TableName  = "ImportRollingWeekCases"
###


$csvFile = Get-ChildItem -Path $csvPath | where {$_.name -like "RollingFilter*"}

if (!($csvFile)) {
$query = "IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[DBO].[$TableName]') AND type in (N'U')) 
...
$csvFile
$fileInput = Import-CSV -Delimiter "," $csvFile.FullName

$dt = $fileInput | Out-DataTable
...
# Import Data into Tempory Table

# create table structure
Add-SqlTable -ServerInstance $sqlserver -Database $database -TableName $TableName -DataTable $dt

# fill table
Write-DataTable -ServerInstance $sqlserver -Database $database -TableName $TableName -Data $dt
